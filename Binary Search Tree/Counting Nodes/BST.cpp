...
int main(int argc, char **argv)
{
    ...
    while (!quit)
    {
        ...
        if (refresh)
        {
            ...
            //******************************
            int num_of_nodes = my_tree.count_node();
            string tmpStr = to_string(num_of_nodes);
            int len = tmpStr.length();

            char charArray[len + 1];
            strcpy(charArray, tmpStr.c_str());

            draw_text(renderer, 40, 40, charArray, Red, alignCenter);
            //******************************
